...
const CONTACT_EMAIL = "cs@kcf-korea.org";

const APPEAL_MAILTO =
  `mailto:${CONTACT_EMAIL}?subject=` +
  encodeURIComponent("[SURE-CHECK] 소명·정정 요청") +
  "&body=" +
  encodeURIComponent(
    "진단 결과 또는 공개 사례에 대한 소명·정정 요청 내용을 적어 주세요.",
  );

const INQUIRY_MAILTO =
  `mailto:${CONTACT_EMAIL}?subject=` +
  encodeURIComponent("[SURE-CHECK] 개선 안내 문의") +
  "&body=" +
  encodeURIComponent(
    "개인정보 수집 고지 개선 또는 SURE-CHECK 운영·협력 문의 내용을 적어 주세요.",
  );
    ...
    <div className="flex min-h-screen flex-col bg-[#f8fafc]">
      ...
      <main className="flex-1">
        ...
        <div className="mx-auto max-w-[72rem] space-y-10 px-5 py-10 md:space-y-14 md:px-8 md:py-14">
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              {PRINCIPLES.map((item, index) => {
                ...
                return (
                  <article
                    key={item.title}
                    className={`rounded-2xl border border-slate-200 bg-white p-5 ${index === PRINCIPLES.length - 1 ? "md:col-span-2" : ""}`}
                  >
                    <div className="flex items-start gap-3">
                      <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-teal-50 text-teal-800">
                        <Icon className="h-5 w-5" aria-hidden />
          ...
          <section id="inquiry" className="scroll-mt-8">
            <SectionHeading title="소명·정정 및 문의" />
            <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-5 md:p-8">
              <div className="max-w-3xl space-y-3 text-sm leading-relaxed text-slate-600 md:text-base">
                <p>
                  SURE-CHECK 진단 결과에 대해 기관·기업이 이견이 있거나 이미
                  개선 조치를 완료한 경우, 소명 또는 정정 요청을 할 수 있습니다.
                </p>
                <p>
                  제출된 소명 내용은 검토 후 진단 상태, 개선안내 상태, 공개 사례
                  반영 여부 등에 반영될 수 있습니다.
                </p>
              </div>
              <ul className="mt-5 grid gap-2 sm:grid-cols-2">
                {INQUIRY_ITEMS.map((item) => (
                  ...
                  >
                    <Mail
                      className="mt-0.5 h-4 w-4 shrink-0 text-teal-800"
                      aria-hidden
                    />
                    {item}
                  </li>
