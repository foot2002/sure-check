...
/** Pause crawl/diagnosis crons. Keep false in production unless ops explicitly halt. */
export const COLLECTOR_OPS_HALTED = false;
 ...
 * - Revalidate paused.
 * - Official site every 2 hours as Hobby once-daily slots, 8 orgs/run
 *   (~96/일 이론, 겹치면 skip). Total crons stay at 45 (last successful deploy).
 * - Diagnosis dispatch ~hourly in the existing worker window, official_site only.
 * - Scan worker 22 proven once-daily slots (scanBatch=3).
...
export const COLLECTOR_OPS_SCHEDULE_NOTES = {
  ...
  diagnosisDispatch: "기존 worker 창에서 official_site 대상 재개",
  scanWorker: "기존 22 once-daily 슬롯(scanBatch=3) 재개",
} as const;
