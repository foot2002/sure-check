...
import type { GoogleFormsExtractorInput } from "@/lib/extractors/googleFormsTypes";
import type {
  DetectedCategory,
  NormalizedForm,
  NormalizedPage,
...
function buildPages(
  ...
  parsedQuestions: ReturnType<typeof parseGoogleFormsHtml>["questions"],
): NormalizedPage[] {
...
export function extractGoogleForms(input: GoogleFormsExtractorInput): NormalizedForm {
  ...
  const extractionWarnings = [
    ...
    parsed.extractionMethod === "dom_fallback"
      ...
      : undefined,
    parsed.closedForm ? "응답이 종료된 Google Forms로 보입니다." : undefined,
  ...
  return {
    ...
    pages,
    partialScan: parsed.partialScan || parsed.extractionMethod === "dom_fallback",
    isLimited,
    ...
    metadata: {
      ...
      headings: pages.map((p) => p.title).filter((t): t is string => Boolean(t)),
      extractionWarnings,
    },
    management: {
