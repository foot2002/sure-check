...
function main() {
  ...
  check("today vs total heading", view.includes("오늘 공공 사이트 수집") && view.includes("전체 누적 통계"));
  check("today date unknown label", view.includes("오늘 공공 사이트 날짜 불명 보류"));
  check("total date unknown label", view.includes("전체 날짜 불명 보류"));
  check("today recent eligible", view.includes("오늘 공공 사이트 최근 60일 적격"));
  check("total eligible", view.includes("전체 공공 사이트 적격 설문"));
  ...
  check("quick filters", view.includes("날짜불명 보류") && view.includes("공공 사이트 수집"));
  ...
  check("collapsible logs", view.includes("최근 수집 실행 상세") && view.includes("최근 공공 사이트 수집 로그"));
  ...
  check("source channel public site", collectorSourceChannelKo("official_site") === "공공 사이트");
