...

{
  ...
  assert.ok(view.includes("오늘 공공 사이트 최근 60일 적격"));
  assert.ok(view.includes("전체 공공 사이트 적격 설문"));
  assert.ok(view.includes("전체 누적 통계"));
  assert.ok(view.includes("source_page_url 저장률"));
  assert.ok(view.includes("seed 오매핑 의심"));
  assert.ok(view.includes("15분 간격"));
  ...
  assert.ok(view.includes("네이버 API와 공공 사이트 비교"));
