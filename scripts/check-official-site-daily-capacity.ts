/**
 * Official-site daily capacity: 8 orgs/run × 48 half-hour Hobby slots = 384/day.
 */
...

{
  ...
  assert.equal(OFFICIAL_SITE_WAVES_PER_DAY, 48);
  assert.equal(OFFICIAL_SITE_TARGET_ORGS_PER_DAY, 384);
  assert.equal(estimatedOfficialSiteOrgsPerDay(), 384);
  console.log("  PASS  8 orgs/run × 48 waves = 384 orgs/day");
}

{
  ...
  assert.equal(countCronJobsForPath(crons, OFFICIAL_SITE_CRON_PATH), 48);
  ...
  assert.equal(officialSiteWavesPerDayFromCrons(crons), 48);
  console.log("  PASS  48 Hobby once-daily official-site crons (30-minute cadence)");
}
...

{
  ...
  assert.ok(view.includes("30분 간격 24시간"));
